		...
		<div class="col-sm-8 text-left shadowed bgw">
			...
			<div class="table-responsive ">
				<table class="table">
					<thread>
					<tr class="ordersList">
						<td class="col-xs-1"><spring:message code="label.invoice" />
						</td>
						<td class="col-xs-3"><spring:message code="label.date" /></td>
						<td class="col-xs-2"><spring:message code="label.sum" /></td>
						<td class="col-xs-4"><spring:message code="label.client" />
						</td>
						<td class="col-xs-2"><spring:message code="label.orderstatus" />
						</td>
					</tr>
					</thread>
					...
					<tbody>
						...
						<c:forEach var="porder" items="${porders }" varStatus="i">
							...
							<tr>
								<td class="col-xs-1"><span> <a
										href="${context }/order/${order.id }"><c:out
												value="${order.id }" /></a>
								</span></td>
								<td class="col-xs-3"><siteDir:date
										date="${order.creationDate}" /></td>
								<td class="col-xs-2"><c:out value="${porder.subtotal } EUR" />
								</td>
								<td class="col-xs-4"><c:out
										value="${order.first_name } ${order.last_name }" /></td>
								<td class="col-xs-2"><c:out
